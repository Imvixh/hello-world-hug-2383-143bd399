...
function DashboardPage() {
  return (
    <div className="grodo-dark min-h-screen bg-background text-foreground">
      <div className="flex min-h-screen">
        {/* sidebar */}
        <aside className="hidden w-[236px] shrink-0 flex-col border-r border-border bg-card/60 px-3.5 py-5 lg:flex">
          <Link to="/" className="mb-7 flex items-center gap-2 px-2">
            <GrodoMark className="h-7 w-7" id="dash-mark" />
          ...
          <nav className="space-y-1">
            {NAV.map(({ label, icon: Icon, badge, active }) => (
              <button
                key={label}
                type="button"
                className={`flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-[0.88rem] font-medium transition-colors ${
                  active
          ...
          <div className="mt-auto rounded-2xl border border-border bg-white/[0.03] p-4">
            <p className="text-[0.85rem] font-bold">{previewUser.plan}</p>
            <p className="mt-1 text-[0.78rem] leading-[1.5] text-muted-foreground">
              Upgrade to unlock advanced analytics and AI tools.
            </p>
            <Link
              to="/pricing"
              className="cta-gradient mt-3 flex items-center justify-center rounded-lg px-3 py-2 text-[0.8rem] font-semibold text-primary-foreground"
            >
              View plans
            </Link>
          </div>
        ...
        <div className="min-w-0 flex-1">
          <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 border-b border-border px-5 py-4 sm:px-8">
            ...
            <div className="flex shrink-0 items-center gap-2.5">
              ...
              <span className="flex items-center gap-2 rounded-xl border border-border px-2.5 py-1.5">
              ...
              <div className="rounded-2xl border border-border bg-card p-6">
                ...
                <div className="mt-6 flex h-[240px] flex-col items-center justify-center rounded-xl border border-dashed border-border text-center">
                  ...
                  <p className="mt-1.5 max-w-[300px] text-[0.82rem] leading-[1.55] text-muted-foreground">
                    Once you connect a channel and publish, your reach and engagement will appear
                    here.
                  </p>
                </div>
              </div>
                ...
                <div className="mt-4 space-y-2.5">
                  {[
                    ...
                    <button
                      key={p}
                      type="button"
                      className="w-full rounded-xl border border-border bg-white/[0.03] px-4 py-3 text-left text-[0.83rem] transition-colors hover:border-white/25"
                    >
                      {p}
                    </button>
                  ))}
